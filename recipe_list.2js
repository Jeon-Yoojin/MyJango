...
const Recipe_youtube_list = ()=>{
  ...
  const videolist = _getThumbnail("마라탕");
  const getData = () => {
    videolist.then((val) => {
...
const [BMlist, setBMlist] = useState([]);
  ...
  WHERE trim(A.ingredient_name) =? OR trim(A.ingredient_name) =? OR trim(A.ingredient_name) =? OR trim(A.ingredient_name) =? 
  GROUP BY A.recipe_id HAVING count(A.ingredient_name) >= 3
  ORDER BY count(A.ingredient_name) DESC) C 
  WHERE B.id=C.recipe_id ORDER BY C.recipe_count DESC'*/
...
/>
  ...
  <View style = {styles.buttonview}>
    ...
    <TouchableOpacity>
    <Text onPress={getbookmark}>bookmark</Text>
  </TouchableOpacity>
  <TouchableOpacity>
    <Text onPress={()=>{console.log("bm임니다~~~~ ",BMlist); 
    console.log("파이널잉그입니다~~~~~",finalIngList)}} style={{fontSize:30}}>잘되고있는거야?</Text>
  </TouchableOpacity>
  ...
  <TouchableOpacity>
    <Text onPress={()=>{console.log("rmfotjdly dlrp anjsirh------------",finalFoodList)}}>finalFoodList</Text>
  </TouchableOpacity>
...
{thumbnailList.map((thumbnail, index)=>{
 ...
 /*return(
 <View>
    <Text onPress={()=>{insertData; console.log("아니 왜이러는거예요")}}>인서트</Text>
  <Text onPress={viewIng}>봐야쥐</Text>
  <Text onPress={getbookmark} style={{fontSize:30}}>getbookmark</Text>
  <TouchableOpacity>
    <Text onPress={()=>{modifySL; console.log("bm임니다~~~~ ",BMlist); 
    console.log("파이널잉그입니다~~~~~",finalIngList)}} style={{fontSize:30}}>BMlist</Text>
  </TouchableOpacity>
  <TouchableOpacity>
    <Text onPress={searchWith} style={{fontSize:30}}>searchWith</Text>
  </TouchableOpacity>
  <TouchableOpacity>
    <Text onPress={()=>{searchWith; console.log("여기서 나오는거임",finalFoodList)}}>finalFoodList</Text>
  </TouchableOpacity>
  <Text onPress={console.log("아제발요")}>showMethefinalIngList</Text>
</View>
 )*/
};
