...
export default function SearchLike(recipeName) {
    let date = getToday();
    let nearExp = getNearExp();
    let searchLike = false;

  const [finalFoodList, setFinalFoodList] = useState([]);
  const [finalIngList, setFinalIngList] = useState(['감자', '배추김치']);
    ...
    const Recipe_youtube_list = ()=>{
      ...
      const videolist = _getThumbnail(recipeName);
     ...
     return(<View style={{alignItems:'center'}}>
       ...
       {thumbnailList.map((thumbnail, index)=>{
                        return(thumbnailList[index] ? <TouchableOpacity onPress={()=>thumbnailClicked(thumbnail.vId, thumbnail.name)} key={index}><Image source={{uri: thumbnail.imguri}} key={index} style={{width: fullWidth, height: 240}}/></TouchableOpacity> : <Text>Loading</Text>)
                    })
        }
       </ScrollView>
     </View>)
    ...
    const thumbnailClicked = (vId, name)=>{
